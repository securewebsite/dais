...
final class Search {
	...
	public function add($language, $type, $id, $text = false) {
		...
		\DB::query("
			INSERT INTO " . \DB::p()->prefix . "search_index 
			SET 
	...
	public function delete($type, $id) {
		\DB::query("
			DELETE FROM " . \DB::p()->prefix . "search_index 
	...
	private function category($id) {
		$query = \DB::query("
			...
			FROM " . \DB::p()->prefix . "category c 
			LEFT JOIN " . \DB::p()->prefix . "category_description cd 
	...
	private function page($id) {
		...
		$query = \DB::query("
			...
			FROM " . \DB::p()->prefix . "page p 
			LEFT JOIN " . \DB::p()->prefix . "page_description pd 
	...
	private function blog_category($id) {
		$query = \DB::query("
			SELECT c.category_id, c.parent_id, cd.name, cd.description  
			FROM " . \DB::p()->prefix . "blog_category c 
			LEFT JOIN " . \DB::p()->prefix . "blog_category_description cd 
			ON (c.category_id = cd.category_id) 
			WHERE c.category_id = '" . (int)$id . "' 
			AND c.status        = '1' 
			AND cd.language_id  = '" . (int)\Config::get('config_language_id') . "'
	...
	private function post($id) {
		...
		$query = \DB::query("
			...
			FROM " . \DB::p()->prefix . "blog_post p 
			LEFT JOIN " . \DB::p()->prefix . "blog_post_description pd 
		...
		switch(\Config::get('active.facade')):
			...
			case ADMIN_FACADE:
				$query = \DB::query("
					...
					FROM " . \DB::p()->prefix . "customer_group
				");
