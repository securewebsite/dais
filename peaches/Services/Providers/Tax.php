		...
		if ($this->shipping_address):
			$tax_query = \DB::query("
				SELECT 
					...
					tr2.type, 
					tr1.priority 
				FROM " . \DB::p()->prefix . "tax_rule tr1 
				LEFT JOIN " . \DB::p()->prefix . "tax_rate tr2 
					ON (tr1.tax_rate_id = tr2.tax_rate_id) 
				INNER JOIN " . \DB::p()->prefix . "tax_rate_to_customer_group tr2cg 
					ON (tr2.tax_rate_id = tr2cg.tax_rate_id) 
				LEFT JOIN " . \DB::p()->prefix . "zone_to_geo_zone z2gz 
					ON (tr2.geo_zone_id = z2gz.geo_zone_id) 
				LEFT JOIN " . \DB::p()->prefix . "geo_zone gz 
