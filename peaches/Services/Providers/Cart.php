        ...
        if (!$this->data):
            foreach (\Session::p()->data['cart'] as $key => $quantity):
                ...
                $product_query = \DB::query("
                    SELECT * 
                    FROM " . \DB::p()->prefix . "product p 
                    LEFT JOIN " . \DB::p()->prefix . "product_description pd 
                ...
                if ($product_query->num_rows):
                    ...
                    foreach ($options as $product_option_id => $option_value):
                        $option_query = \DB::query("
                            SELECT 
                                ...
                                od.name, 
                                o.type 
                            FROM " . \DB::p()->prefix . "product_option po 
                            LEFT JOIN `" . \DB::p()->prefix . "option` o 
                                ON (po.option_id = o.option_id) 
                            LEFT JOIN " . \DB::p()->prefix . "option_description od 
                            ...
                            if ($option_query->row['type'] == 'select' || $option_query->row['type'] == 'radio' || $option_query->row['type'] == 'image'):
                                $option_value_query = \DB::query("
                                    SELECT 
                                        ...
                                        pov.weight, 
                                        pov.weight_prefix 
                                    FROM " . \DB::p()->prefix . "product_option_value pov 
                                    LEFT JOIN " . \DB::p()->prefix . "option_value ov 
                                        ON (pov.option_value_id = ov.option_value_id) 
                                    LEFT JOIN " . \DB::p()->prefix . "option_value_description ovd 
                    ...
                    $product_discount_query = \DB::query("
                        SELECT price 
                        FROM " . \DB::p()->prefix . "product_discount 
                    ...
                    $product_special_query = \DB::query("
                        SELECT price 
                        FROM " . \DB::p()->prefix . "product_special 
                        WHERE product_id = '" . (int)$product_id . "' 
                        AND customer_group_id = '" . (int)$customer_group_id . "' 
                        AND ((date_start = '0000-00-00' OR date_start < NOW()) 
                        AND (date_end = '0000-00-00' OR date_end > NOW())) 
                    ...
                    $product_reward_query = \DB::query("
                        SELECT points 
                        FROM " . \DB::p()->prefix . "product_reward 
                        WHERE product_id = '" . (int)$product_id . "' 
                        AND customer_group_id = '" . (int)$customer_group_id . "'
                    ");
                    ...
                    $download_query = \DB::query("
                        SELECT * 
                        FROM " . \DB::p()->prefix . "product_to_download p2d 
                        LEFT JOIN " . \DB::p()->prefix . "download d 
                            ON (p2d.download_id = d.download_id) 
                        LEFT JOIN " . \DB::p()->prefix . "download_description dd 
                    ...
                    $recurring_query = \DB::query("
                        SELECT * 
                        FROM " . \DB::p()->prefix . "recurring p 
                        JOIN " . \DB::p()->prefix . "product_recurring pp 
                            ON (pp.recurring_id = p.recurring_id) 
                            AND (pp.product_id = " . (int)$product_query->row['product_id'] . ") 
                        JOIN " . \DB::p()->prefix . "recurring_description pd 
