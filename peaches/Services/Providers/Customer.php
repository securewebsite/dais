...
class Customer {
    ...
    public function login($email, $password, $override = false) {
        if ($override):
            $customer_query = \DB::query("
                SELECT * FROM " . \DB::p()->prefix . "customer 
        ...
        if ($customer_query->num_rows):
            ...
            $customer_group_query = \DB::query("
                SELECT name 
                FROM " . \DB::p()->prefix . "customer_group_description 
    ...
    public function getBalance() {
        $query = \DB::query("
            ...
            FROM " . \DB::p()->prefix . "customer_credit 
            WHERE customer_id = '" . (int)$this->customer_id . "'
        ");
    ...
    public function addReward($customer_id, $description, $points, $order_id = 0) {
        \DB::query("
            INSERT INTO " . \DB::p()->prefix . "customer_reward 
            SET 
    ...
    public function addCommission($customer_id, $order_id, $description, $amount) {
        \DB::query("
            INSERT INTO " . \DB::p()->prefix . "customer_commission 
            SET 
                customer_id = '" . (int)$customer_id . "', 
                order_id    = '" . (int)$order_id . "', 
                description = '" . \DB::escape($description) . "', 
    ...
    public function processMembership($products) {
        ...
        foreach ($products as $product_id):
            $query = \DB::query("
                ...
                FROM " . \DB::p()->prefix . "product_recurring 
