...
class Rss extends Controller {
    ...
    public function index() {
        ...
        foreach ($posts as $post) {
            if ($post['description']) {
                ...
                if ($post['image']) {
                    $output.= '<g:image_link>' . ToolImage::resize($post['image'], 500, 500) . '</g:image_link>';
                } else {
                    $output.= '<g:image_link>' . ToolImage::resize('no_image.jpg', 500, 500) . '</g:image_link>';
                }
    ...
    protected function getPath($parent_id, $current_path = '') {
        $category_info = ContentCategory::getCategory($parent_id);
