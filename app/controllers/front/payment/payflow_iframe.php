...
class PayflowIframe extends Controller {
    public function index() {
        ...
        if ($this->config->get('payflow_iframe_transaction_method') == 'sale') {
            $transaction_type = 'S';
        } else {
            $transaction_type = 'A';
        }
        ...
        $data['checkout_method'] = $this->config->get('payflow_iframe_checkout_method');
        
        $data = $this->theme->listen(__CLASS__, __FUNCTION__, $data);
        
        return $this->theme->view('payment/payflow_iframe', $data);
    }
    ...
    public function pp_return() {
        ...
        $this->response->setOutput($this->theme->view('payment/payflow_iframe_return', $data));
    }
    ...
    public function pp_cancel() {
        ...
        $this->response->setOutput($this->theme->view('payment/payflow_iframe_cancel', $data));
    }
    ...
    public function pp_error() {
        ...
        $this->response->setOutput($this->theme->view('payment/payflow_iframe_error', $data));
    }
    ...
    public function pp_post() {
        ...
        $this->model_payment_payflow_iframe->log('POST: ' . print_r($this->request->post, 1));
        
        $order_id = $this->model_payment_payflow_iframe->getOrderId($this->request->post['SECURETOKENID']);
        ...
        if ($order_id) {
            ...
            if ($order_info['order_status_id'] == 0 && $response_params['RESULT'] == '0' && $this->request->post['RESULT'] == 0) {
                $this->model_checkout_order->confirm($order_id, $this->config->get('payflow_iframe_order_status_id'));
                ...
                $this->model_payment_payflow_iframe->updateOrder($posted);
                ...
                $this->model_payment_payflow_iframe->addTransaction($posted);
            }
        }
