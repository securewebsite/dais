...
class Fedex extends Model {
    function getQuote($address) {
        ...
        if ($status):
            ...
            $country_info = LocaleCountry::getCountry(Config::get('config_country_id'));
            
            Theme::model('locale/zone');
            $zone_info = LocaleZone::getZone(Config::get('config_zone_id'));
            ...
            $xml.= '						<ns1:PhoneNumber>' . Customer::getTelephone() . '</ns1:PhoneNumber>';
            $xml.= '					</ns1:Contact>';
            $xml.= '					<ns1:Address>';
                ...
                foreach ($rate_reply_details as $rate_reply_detail):
                    ...
                    if (in_array(strtoupper($code), Config::get('fedex_service'))):
                        ...
                        $quote_data[$code] = array(
                            ...
                            'text'         => Currency::format(Tax::calculate(Currency::convert($cost, $currency, Currency::getCode()), Config::get('fedex_tax_class_id'), Config::get('config_tax')), Currency::getCode(), 1.0000000)
                        );
                    endif;
                endforeach;
            endif;
