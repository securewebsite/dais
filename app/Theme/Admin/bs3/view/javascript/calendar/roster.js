...
$(document).on('click', '#add_waitlist', function() {
	if ($('input[name=\'customer\']').val() != "") {
		...
		$.ajax({
			url: 'index.php?route=calendar/event/add_to_wait_list&token=<?= $token; ?>',
			type: 'POST',
			dataType: 'json',
			data: dataString,
...
$(document).on('click', '#add_attendee', function() {
	if ($('input[name=\'customer\']').val() != "") {
		var dataString = 'event_id=<?= $event_id; ?>&attendee_id=' + $('input[name=\'attendee_id\']').val();
		$.ajax({
			url: 'index.php?route=calendar/event/insert_attendee&token=<?= $token; ?>',
			type: 'POST',
			dataType: 'json',
			data: dataString,
			success: function(json) {
...
$('input[name="customer"]').typeahead({
	source:function(q,process){
		return $.getJSON('index.php?route=calendar/event/autocomplete&token='+token+'&name='+encodeURIComponent(q),function(json){
			var data=[];
