...
<div id="content">
	...
	<?php if ($coupon_status || $giftcard_status || $reward_status) { ?>
			...
			<table class="radio">
				...
				<?php if ($giftcard_status) { ?>
					<tr class="highlight">
						<td>
							<div class="radio radio-inline">
							<?php if ($next == 'gift_card') { ?>
								<input type="radio" name="next" value="gift_card" id="use_gift_card" checked="">
							<?php } else { ?>
								<input type="radio" name="next" value="gift_card" id="use_gift_card">
							<?php } ?>
							</div>
						</td>
						<td><label for="use_gift_card"><?= $lang_text_use_gift_card; ?></label></td>
					</tr>
				<?php } ?>
		...
		<div class="cart-discounts">
			...
			<div id="gift_card" class="content" style="display: <?= ($next == 'gift_card' ? 'block' : 'none'); ?>;">
				<form action="<?= $action; ?>" method="post" enctype="multipart/form-data">
					<?= $lang_entry_gift_card; ?>&nbsp;
					<input type="text" name="gift_card" value="<?= $gift_card; ?>">
					<input type="hidden" name="next" value="gift_card">
					&nbsp;
					<input type="submit" value="<?= $lang_button_gift_card; ?>" class="btn btn-default">
				</form>
			</div>
...
<?php } ?>
	...
	<div class="checkout-product">
		<table>
			...
			<tr>
				<td class="name"><?= $gift_card['description']; ?></td>
				<td class="model"></td>
				<td class="quantity">1</td>
				<td class="price"><?= $gift_card['amount']; ?></td>
				<td class="total"><?= $gift_card['amount']; ?></td>
			</tr>
			<?php } ?>
			</tbody>
			<tfoot>
