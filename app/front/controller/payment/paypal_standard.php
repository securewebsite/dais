...
class PaypalStandard extends Controller {
    public function index() {
        ...
        $data['testmode'] = $this->config->get('paypal_standard_test');
        
        if (!$this->config->get('paypal_standard_test')) {
        ...
        if ($order_info) {
            $data['business'] = $this->config->get('paypal_standard_email');
            ...
            $data['notify_url'] = $this->url->link('payment/paypal_standard/callback', '', 'SSL');
            $data['cancel_return'] = $this->url->link('checkout/checkout', '', 'SSL');
            
            if (!$this->config->get('paypal_standard_transaction')) {
            ...
            $this->theme->loadjs('javascript/payment/paypal_standard', $data);
            
            $data = $this->theme->listen(__CLASS__, __FUNCTION__, $data);
            
            $data['javascript'] = $this->theme->controller('common/javascript');
            
            return $this->theme->view('payment/paypal_standard', $data);
        }
    }
    ...
    public function callback() {
        ...
        if ($order_info) {
            ...
            if ($this->config->get('paypal_standard_debug')) {
            ...
            if ((strcmp($response, 'VERIFIED') == 0 || strcmp($response, 'UNVERIFIED') == 0) && isset($this->request->post['payment_status'])) {
                ...
                switch ($this->request->post['payment_status']) {
                    case 'Canceled_Reversal':
                        $order_status_id = $this->config->get('paypal_standard_canceled_reversal_status_id');
                        break;

                    case 'Completed':
                        if ((strtolower($this->request->post['receiver_email']) == strtolower($this->config->get('paypal_standard_email'))) && ((float)$this->request->post['mc_gross'] == $this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false))) {
                            $order_status_id = $this->config->get('paypal_standard_completed_status_id');
                        } else {
                    ...
                    case 'Denied':
                        $order_status_id = $this->config->get('paypal_standard_denied_status_id');
                        break;

                    case 'Expired':
                        $order_status_id = $this->config->get('paypal_standard_expired_status_id');
                        break;

                    case 'Failed':
                        $order_status_id = $this->config->get('paypal_standard_failed_status_id');
                        break;

                    case 'Pending':
                        $order_status_id = $this->config->get('paypal_standard_pending_status_id');
                        break;

                    case 'Processed':
                        $order_status_id = $this->config->get('paypal_standard_processed_status_id');
                        break;

                    case 'Refunded':
                        $order_status_id = $this->config->get('paypal_standard_refunded_status_id');
                        break;

                    case 'Reversed':
                        $order_status_id = $this->config->get('paypal_standard_reversed_status_id');
                        break;

                    case 'Voided':
                        $order_status_id = $this->config->get('paypal_standard_voided_status_id');
                        break;
                }
