...
class PaypalExpress extends Controller {
    public function index() {
        ...
        $data['button_continue_action'] = $this->url->link('payment/paypal_express/checkout', '', 'SSL');
        ...
        $this->theme->loadjs('javascript/payment/paypal_express', $data);
        
        $data = $this->theme->listen(__CLASS__, __FUNCTION__, $data);
        
        $data['javascript'] = $this->theme->controller('common/javascript');
        
        return $this->theme->view('payment/paypal_express', $data);
    }
    ...
    public function express() {
        ...
        $data = array(
            ...
            'MAXAMT'             => $max_amount, 
            'RETURNURL'          => $this->url->link('payment/paypal_express/expressReturn', '', 'SSL'), 
            'CANCELURL'          => $this->url->link('checkout/cart', '', 'SSL'), 
            'REQCONFIRMSHIPPING' => 0, 
            'NOSHIPPING'         => $shipping, 
            'ALLOWNOTE'          => $this->config->get('paypal_express_allow_note'), 
            'LOCALECODE'         => 'EN', 
            'LANDINGPAGE'        => 'Login', 
            'HDRIMG'             => $this->model_tool_image->resize($this->config->get('paypal_express_logo'), 790, 90), 
            'PAYFLOWCOLOR'       => $this->config->get('paypal_express_page_colour'), 
            'CHANNELTYPE'        => 'Merchant'
        );
        
        if (isset($this->session->data['paypal_express_login']['seamless']['access_token']) && (isset($this->session->data['paypal_express_login']['seamless']['customer_id']) && $this->session->data['paypal_express_login']['seamless']['customer_id'] == $this->customer->getId()) && $this->config->get('paypal_express_login_seamless')) {
        ...
        if (!isset($result['TOKEN'])) {
             ...
             */
            if ($this->config->get('paypal_express_debug') == 1) {
    ...
    public function expressReturn() {
        ...
        $this->response->redirect($this->url->link('payment/paypal_express/expressConfirm', '', 'SSL'));
    }
    ...
    public function expressConfirm() {
        ...
        $this->breadcrumb->add('lang_text_title', 'payment/paypal_express/express', '', true, 'SSL');
        $this->breadcrumb->add('lang_express_text_title', 'payment/paypal_express/expressConfirm', '', true, 'SSL');
        ...
        $data['action'] = $this->url->link('payment/paypal_express/expressConfirm', '', 'SSL');
            ...
            if (!empty($shipping_address)) {
                ...
                if (!empty($results)) {
                    ...
                    if (!empty($quote_data)) {
                        ...
                        $data['action_shipping'] = $this->url->link('payment/paypal_express/shipping', '', 'SSL');
                    } else {
                        unset($this->session->data['shipping_methods']);
                        unset($this->session->data['shipping_method']);
        ...
        $data['action_confirm'] = $this->url->link('payment/paypal_express/expressComplete', '', 'SSL');
        ...
        $this->theme->loadjs('javascript/payment/paypal_express_confirm', $data);
        ...
        $this->response->setOutput($this->theme->view('payment/paypal_express_confirm', $data));
    }
    ...
    public function expressComplete() {
        ...
        if ($redirect == '') {
            ...
            $paypal_data = array(
                ...
                'PAYMENTREQUEST_0_NOTIFYURL' => $this->url->link('payment/paypal_express/ipn', '', 'SSL'), 
                'RETURNFMFDETAILS'           => 1
            );
            ...
            if ($result['ACK'] == 'Success') {
                ...
                switch ($result['PAYMENTINFO_0_PAYMENTSTATUS']) {
                    case 'Canceled_Reversal':
                        $order_status_id = $this->config->get('paypal_express_canceled_reversal_status_id');
                        break;

                    case 'Completed':
                        $order_status_id = $this->config->get('paypal_express_completed_status_id');
                        break;

                    case 'Denied':
                        $order_status_id = $this->config->get('paypal_express_denied_status_id');
                        break;

                    case 'Expired':
                        $order_status_id = $this->config->get('paypal_express_expired_status_id');
                        break;

                    case 'Failed':
                        $order_status_id = $this->config->get('paypal_express_failed_status_id');
                        break;

                    case 'Pending':
                        $order_status_id = $this->config->get('paypal_express_pending_status_id');
                        break;

                    case 'Processed':
                        $order_status_id = $this->config->get('paypal_express_processed_status_id');
                        break;

                    case 'Refunded':
                        $order_status_id = $this->config->get('paypal_express_refunded_status_id');
                        break;

                    case 'Reversed':
                        $order_status_id = $this->config->get('paypal_express_reversed_status_id');
                        break;

                    case 'Voided':
                        $order_status_id = $this->config->get('paypal_express_voided_status_id');
                        break;
                }
                ...
                $paypal_order_data = array(
                    'order_id'         => $order_id, 
                    'capture_status'   => ($this->config->get('paypal_express_method') == 'Sale' ? 'Complete' : 'NotComplete'), 
                ...
                $paypal_transaction_data = array(
                    ...
                    'transaction_entity'    => ($this->config->get('paypal_express_method') == 'Sale' ? 'payment' : 'auth'), 
    ...
    public function checkout() {
        ...
        $data = array(
            'METHOD'             => 'SetExpressCheckout', 
            'MAXAMT'             => $max_amount, 
            'RETURNURL'          => $this->url->link('payment/paypal_express/checkoutReturn', '', 'SSL'), 
    ...
    public function ipn() {
        $this->theme->model('payment/paypal_express');
        $this->theme->model('payment/express_ipn');
        ...
        if ((string)$response == "VERIFIED") {
            ...
            if ($transaction) {
                ...
                if ($this->config->get('paypal_express_debug') == 1) $this->log->write('Transaction exists');
                ...
                if ($this->config->get('paypal_express_debug') == 1) $this->log->write('Transaction does not exist');
                ...
                if ($parent_transaction) {
                    if ($this->config->get('paypal_express_debug') == 1) $this->log->write('Parent transaction exists');
                    ...
                    if ($this->config->get('paypal_express_debug') == 1) $this->log->write('Parent transaction not found');
                }
            }
    ...
    public function shipping() {
        ...
        $this->response->redirect($this->url->link('payment/paypal_express/expressConfirm'));
    }
    ...
    protected function validateGiftcard() {
        ...
        if (!$gift_card_info) {
            $error = $this->language->get('lang_error_gift_card');
        }
        
        if (!$error) {
            return true;
        } else {
            $this->session->data['error_warning'] = $this->language->get('lang_error_gift_card');;
            return false;
        }
    }
    ...
    public function recurringButtons() {
        ...
        if ($recur['status'] == 2 || $recur['status'] == 3) {
            $data['buttons'][] = array(
                ...
                'link' => $this->url->link('payment/paypal_express/recurringCancel', 'recurring_id=' . $this->request->get['recurring_id'], 'SSL')
            );
        }
