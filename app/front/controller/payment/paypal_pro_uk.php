...
class PaypalProUk extends Controller {
    public function index() {
        ...
        $this->theme->loadjs('javascript/payment/paypal_pro_uk', $data);
        
        $data = $this->theme->listen(__CLASS__, __FUNCTION__, $data);
        
        $data['javascript'] = $this->theme->controller('common/javascript');
        
        return $this->theme->view('payment/paypal_pro_uk', $data);
    }
    
    public function send() {
        if (!$this->config->get('paypal_pro_uk_transaction')) {
        ...
        $request.= '&USER=' . urlencode($this->config->get('paypal_pro_uk_username'));
        $request.= '&PWD=' . urlencode($this->config->get('paypal_pro_uk_password'));
        $request.= '&SIGNATURE=' . urlencode($this->config->get('paypal_pro_uk_signature'));
        ...
        if (!$this->config->get('paypal_pro_uk_test')) {
        ...
        if (($response_info['ACK'] == 'Success') || ($response_info['ACK'] == 'SuccessWithWarning')) {
            ...
            $this->model_checkout_order->update($this->session->data['order_id'], $this->config->get('paypal_pro_uk_order_status_id'), $message);
