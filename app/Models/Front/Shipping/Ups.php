...
class Ups extends Model {
    function getQuote($address) {
        ...
        if ($status):
            $weight      = $this->weight->convert($this->cart->getWeight(), Config::get('config_weight_class_id'), Config::get('ups_weight_class_id'));
                    ...
                    foreach ($rated_shipments as $rated_shipment):
                        ...
                        if (Config::get('ups_' . strtolower(Config::get('ups_origin')) . '_' . $code)):
                            $quote_data[$code] = array(
                                ...
                                'text'         => $this->currency->format($this->tax->calculate($this->currency->convert($cost, $currency, $this->currency->getCode()), Config::get('ups_tax_class_id'), Config::get('config_tax')), $this->currency->getCode(), 1.0000000)
                            );
                        endif;
                    endforeach;
                endif;
