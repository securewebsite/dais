...
class BlogCategory extends Controller {
    ...
    public function index() {
        ...
        } elseif ($this->config->get('blog_category_widget')) {
            $data['widgets'] = $this->config->get('blog_category_widget');
        }
        ...
        $this->theme->loadjs('javascript/widget/blog_category', $data);
        
        $data = $this->theme->listen(__CLASS__, __FUNCTION__, $data);
        
        $data = $this->theme->render_controllers($data);
        
        $this->response->setOutput($this->theme->view('widget/blog_category', $data));
    }
    
    protected function validate() {
        if (!$this->user->hasPermission('modify', 'widget/blog_category')) {
