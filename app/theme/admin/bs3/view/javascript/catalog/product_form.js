...
a.typeahead({
	source:function(q,process){
		return $.getJSON('catalog/option/autocomplete/filter_name/'+encodeURIComponent(q),function(json){
			var data=[];
...
$('input[name="customer"]').typeahead({
	source:function(q,process){
		return $.getJSON('catalog/product/autouser/name/'+encodeURIComponent(q),function(json){
			var data=[];
			$.each(json,function(i,item){
				mapped[item.name]=item;
				data.push(item.name);
