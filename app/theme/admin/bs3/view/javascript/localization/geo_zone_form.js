...
function addGeoZone(){
	...
	html += '<td><select name="zone_to_geo_zone['+zone_to_geo_zone_row+'][country_id]" id="country'+zone_to_geo_zone_row+'" onchange="$(\'#zone'+zone_to_geo_zone_row+'\').load(\'index.php?route=localization/geo_zone/zone&token=<?= $token; ?>&country_id=\'+this.value+\'&zone_id=0\');" class="form-control">';
	...
	$('#zone'+zone_to_geo_zone_row).load('index.php?route=localization/geo_zone/zone&token=<?= $token; ?>&country_id='+$('#country'+zone_to_geo_zone_row).val()+'&zone_id=0');
