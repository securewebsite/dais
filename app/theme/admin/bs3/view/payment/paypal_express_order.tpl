	...
	<tr>
		...
		<td>
			...
			<?php if ($paypal_order['capture_status'] != 'Complete'){ ?>&nbsp;&nbsp;
				<a onclick="doVoid();" class="button paypal_capture" id="btn_void"><?= $lang_button_void; ?></a>
	...
	<tr class="paypal_capture">
		...
		<td>
			...
			<p>
				...
				<a class="btn btn-default" onclick="capture();" id="btn_capture"><?= $lang_button_capture; ?></a>
				...
				<?php foreach ($paypal_order['transactions'] as $transaction){ ?>
					<tr>
						...
						<td><?php if ($transaction['transaction_id']) { ?>
								<a href="<?= $view_link .'&transaction_id='.$transaction['transaction_id']; ?>"><?= $lang_text_view; ?></a>
								<?php if ($transaction['payment_type'] == 'instant' && ($transaction['payment_status'] == 'Completed'|| $transaction['payment_status'] == 'Partially-Refunded')) { ?>
									&nbsp;<a href="<?= $refund_link .'&transaction_id='.$transaction['transaction_id']; ?>"><?= $lang_text_refund; ?></a>
								<?php } ?>
							<?php } else { ?>
									<a onclick="resendTransaction(this); return false;" href="<?= $resend_link . '&paypal_order_transaction_id=' . $transaction['paypal_order_transaction_id']; ?>"><?= $lang_text_resend; ?></a>
							<?php } ?></td>
					</tr>
				<?php } ?>
			</table>
