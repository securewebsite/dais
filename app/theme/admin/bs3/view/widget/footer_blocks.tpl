			...
			<table id="widget" class="table table-bordered table-striped">
				...
				<?php foreach ($widgets as $widget) { ?>
					<tr id="widget-row<?= $widget_row; ?>">
						<td><select name="footer_blocks_widget[<?= $widget_row; ?>][menu_id]" class="form-control">
						...
						<td><select name="footer_blocks_widget[<?= $widget_row; ?>][layout_id]" class="form-control">
						...
						<td><select name="footer_blocks_widget[<?= $widget_row; ?>][position]" class="form-control">
							<?php if ($widget['position'] == 'shop_footer'): ?>
							<option value="shop_footer" selected><?= $lang_text_shop_footer; ?></option>
							<?php else: ?>
							<option value="shop_footer"><?= $lang_text_shop_footer; ?></option>
							<?php endif; ?>
							<?php if ($widget['position'] == 'content_footer'): ?>
							<option value="content_footer" selected><?= $lang_text_content_footer; ?></option>
							<?php else: ?>
							<option value="content_footer"><?= $lang_text_content_footer; ?></option>
							<?php endif; ?>
						</select></td>
						<td><div class="btn-group" data-toggle="buttons">
							<?php if ($widget['status']): ?>
							<label class="btn btn-default active" title="<?= $lang_text_enabled; ?>">
								<input type="radio" name="footer_blocks_widget[<?= $widget_row; ?>][status]" value="1" checked=""><i class="fa fa-play"></i>
							</label>
							<label class="btn btn-default" title="<?= $lang_text_disabled; ?>">
								<input type="radio" name="footer_blocks_widget[<?= $widget_row; ?>][status]" value="0"><i class="fa fa-pause"></i>
							</label>
							<?php else: ?>
							<label class="btn btn-default" title="<?= $lang_text_enabled; ?>">
								<input type="radio" name="footer_blocks_widget[<?= $widget_row; ?>][status]" value="1"><i class="fa fa-play"></i>
							</label>
							<label class="btn btn-default active" title="<?= $lang_text_disabled; ?>">
								<input type="radio" name="footer_blocks_widget[<?= $widget_row; ?>][status]" value="0" checked=""><i class="fa fa-pause"></i>
							</label>
							<?php endif; ?>
						</div></td>
						<td class="text-right">
							<input type="text" name="footer_blocks_widget[<?= $widget_row; ?>][sort_order]" value="<?= $widget['sort_order']; ?>" class="form-control">
						</td>
						<td>
