		...
		<form class="form-inline" action="<?= $delete; ?>" method="post" enctype="multipart/form-data" id="form">
			<table class="table table-bordered table-striped table-hover">
				...
				<tbody data-link="row" class="rowlink">
					...
					<tr>
						<td class="rowlink-skip text-center"><?php if ($gift_card['selected']) { ?>
							<input type="checkbox" name="selected[]" value="<?= $gift_card['gift_card_id']; ?>" checked="">
							<?php } else { ?>
							<input type="checkbox" name="selected[]" value="<?= $gift_card['gift_card_id']; ?>">
							<?php } ?></td>
						<td class="hidden-xs"><?= $gift_card['code']; ?></td>
						<td><?= $gift_card['from']; ?></td>
						<td><?= $gift_card['to']; ?></td>
						<td class="text-right hidden-xs"><?= $gift_card['amount']; ?></td>
						<td class="hidden-xs"><?= $gift_card['theme']; ?></td>
						<td class="hidden-xs text-<?= strtolower($gift_card['status']); ?>"><?= $gift_card['status']; ?></td>
						<td class="hidden-xs"><?= $gift_card['date_added']; ?></td>
						<td class="text-right"><?php foreach ($gift_card['action'] as $action) { ?>
							<a class="btn btn-default" href="<?= $action['href']; ?>">
								<i class="fa fa-pencil-square-o"></i><span class="hidden-xs"> <?= $action['text']; ?></span>
							</a>
							<?php } ?>
							<a class="rowlink-skip btn btn-default" onclick="sendGiftcard('<?= $gift_card['gift_card_id']; ?>','<?= $lang_text_wait; ?>');">
