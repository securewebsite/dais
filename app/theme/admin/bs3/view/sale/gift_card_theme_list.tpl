		...
		<form class="form-inline" action="<?= $delete; ?>" method="post" enctype="multipart/form-data" id="form">
			<table class="table table-bordered table-striped table-hover">
				...
				<tbody data-link="row" class="rowlink">
					<?php if ($gift_card_themes) { ?>
					<?php foreach ($gift_card_themes as $gift_card_theme) { ?>
					<tr>
						<td class="rowlink-skip text-center"><?php if ($gift_card_theme['selected']) { ?>
							<input type="checkbox" name="selected[]" value="<?= $gift_card_theme['gift_card_theme_id']; ?>" checked="">
							<?php } else { ?>
							<input type="checkbox" name="selected[]" value="<?= $gift_card_theme['gift_card_theme_id']; ?>">
							<?php } ?></td>
						<td><?= $gift_card_theme['name']; ?></td>
						<td class="text-right"><?php foreach ($gift_card_theme['action'] as $action) { ?>
