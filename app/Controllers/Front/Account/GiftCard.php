...
class GiftCard extends Controller {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $this->session->data['gift_cards'][mt_rand()] = array(
                'description'       => sprintf(Lang::get('lang_text_for'), $this->currency->format($this->currency->convert($this->request->post['amount'], $this->currency->getCode(), Config::get('config_currency'))), $this->request->post['to_name']), 
                ...
                'amount'            => $this->currency->convert($this->request->post['amount'], $this->currency->getCode(), Config::get('config_currency'))
            );
            
            $this->response->redirect(Url::link('account/gift_card/success'));
        }
        ...
        $data['entry_amount'] = sprintf(Lang::get('lang_entry_amount'), $this->currency->format(Config::get('config_gift_card_min')), $this->currency->format(Config::get('config_gift_card_max')));
        ...
        if (isset($this->request->post['amount'])) {
            ...
            $data['amount'] = $this->currency->format(25, Config::get('config_currency'), false, false);
        }
        ...
        $this->response->setOutput(Theme::view('account/gift_card', $data));
    }
    ...
    protected function validate() {
        ...
        if (($this->currency->convert($this->request->post['amount'], $this->currency->getCode(), Config::get('config_currency')) < Config::get('config_gift_card_min')) || ($this->currency->convert($this->request->post['amount'], $this->currency->getCode(), Config::get('config_currency')) > Config::get('config_gift_card_max'))) {
            $this->error['amount'] = sprintf(Lang::get('lang_error_amount'), $this->currency->format(Config::get('config_gift_card_min')), $this->currency->format(Config::get('config_gift_card_max')) . ' ' . $this->currency->getCode());
        }
