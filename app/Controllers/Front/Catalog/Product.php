...
class Product extends Controller {
    ...
    public function index() {
        ...
        if ($product_info) {
            ...
            if ($product_info['image']) {
                $data['popup'] = $this->model_tool_image->resize($product_info['image'], Config::get('config_image_popup_width'), Config::get('config_image_popup_height'));
                Theme::setOgImage($this->model_tool_image->resize($product_info['image'], 600, 600, 'h'));
            } else {
                $data['popup'] = 'placeholder.png';
            }
            
            if ($product_info['image']) {
                $data['thumb'] = $this->model_tool_image->resize($product_info['image'], Config::get('config_image_thumb_width'), Config::get('config_image_thumb_height'));
            } else {
            ...
            foreach ($results as $result) {
                $data['images'][] = array('popup' => $this->model_tool_image->resize($result['image'], Config::get('config_image_popup_width'), Config::get('config_image_popup_height')), 'thumb' => $this->model_tool_image->resize($result['image'], Config::get('config_image_additional_width'), Config::get('config_image_additional_height')));
            }
            
            if ((Config::get('config_customer_price') && $this->customer->isLogged()) || !Config::get('config_customer_price')) {
                $data['price'] = $this->currency->format($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], Config::get('config_tax')));
            } else {
                $data['price'] = false;
            }
            
            if ((float)$product_info['special']) {
                $data['special'] = $this->currency->format($this->tax->calculate($product_info['special'], $product_info['tax_class_id'], Config::get('config_tax')));
            } else {
                $data['special'] = false;
            }
            ...
            foreach ($discounts as $discount) {
                $data['discounts'][] = array('quantity' => $discount['quantity'], 'price' => $this->currency->format($this->tax->calculate($discount['price'], $product_info['tax_class_id'], Config::get('config_tax'))));
            }
                ...
                if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') {
                    ...
                    foreach ($option['option_value'] as $option_value) {
                        if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
                            if (((Config::get('config_customer_price') && $this->customer->isLogged()) || !Config::get('config_customer_price')) && (float)$option_value['price']) {
                                $price = $this->currency->format($this->tax->calculate($option_value['price'], $product_info['tax_class_id'], Config::get('config_tax')));
                            } else {
                                $price = false;
                            }
            ...
            foreach ($results as $result) {
                if ($result['image']) {
                    $image = $this->model_tool_image->resize($result['image'], Config::get('config_image_related_width'), Config::get('config_image_related_height'));
                } else {
                    $image = false;
                }
            ...
            $this->response->setOutput(Theme::view('catalog/product', $data));
        } else {
            $url = '';
    ...
    public function review() {
        ...
        $this->response->setOutput(Theme::view('catalog/review', $data));
    }
        ...
        if ($product_info && $recurring_info) {
            ...
            if (!$json) {
                ...
                if ($recurring_info['trial_status'] == 1) {
                    $price = $this->currency->format($this->tax->calculate($recurring_info['trial_price'] * $quantity, $product_info['tax_class_id'], Config::get('config_tax')));
                ...
                $price = $this->currency->format($this->tax->calculate($recurring_info['price'] * $quantity, $product_info['tax_class_id'], Config::get('config_tax')));
