...
class PaypalStandard extends Controller {
    public function index() {
        ...
        $data['testmode'] = Config::get('paypalstandard_test');
        
        if (!Config::get('paypalstandard_test')) {
        ...
        if ($order_info) {
            $data['business'] = Config::get('paypalstandard_email');
            ...
            if (!Config::get('paypalstandard_transaction')) {
            ...
            return Theme::view('payment/paypal_standard', $data);
        }
    }
    ...
    public function callback() {
        ...
        if ($order_info) {
            ...
            if (Config::get('paypalstandard_debug')) {
            ...
            if ((strcmp($response, 'VERIFIED') == 0 || strcmp($response, 'UNVERIFIED') == 0) && isset($this->request->post['payment_status'])) {
                ...
                switch ($this->request->post['payment_status']) {
                    case 'Canceled_Reversal':
                        $order_status_id = Config::get('paypalstandard_canceled_reversal_status_id');
                        break;

                    case 'Completed':
                        if ((strtolower($this->request->post['receiver_email']) == strtolower(Config::get('paypalstandard_email'))) && ((float)$this->request->post['mc_gross'] == $this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false))) {
                            $order_status_id = Config::get('paypalstandard_completed_status_id');
                        } else {
                    ...
                    case 'Denied':
                        $order_status_id = Config::get('paypalstandard_denied_status_id');
                        break;

                    case 'Expired':
                        $order_status_id = Config::get('paypalstandard_expired_status_id');
                        break;

                    case 'Failed':
                        $order_status_id = Config::get('paypalstandard_failed_status_id');
                        break;

                    case 'Pending':
                        $order_status_id = Config::get('paypalstandard_pending_status_id');
                        break;

                    case 'Processed':
                        $order_status_id = Config::get('paypalstandard_processed_status_id');
                        break;

                    case 'Refunded':
                        $order_status_id = Config::get('paypalstandard_refunded_status_id');
                        break;

                    case 'Reversed':
                        $order_status_id = Config::get('paypalstandard_reversed_status_id');
                        break;

                    case 'Voided':
                        $order_status_id = Config::get('paypalstandard_voided_status_id');
                        break;
                }
