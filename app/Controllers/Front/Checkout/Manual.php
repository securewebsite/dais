...
class Manual extends Controller {
    public function index() {
        ...
        if ($this->user->isLogged() && $this->user->hasPermission('modify', 'sale/order')) {
            ...
            // Stock
            if (!$this->cart->hasStock() && (!Config::get('config_stock_checkout') || Config::get('config_stock_warning'))) {
            ...
            if ($this->cart->hasShipping()) {
                ...
                $this->tax->setShippingAddress(Config::get('config_country_id'), Config::get('config_zone_id'));
            }
            
            $this->tax->setPaymentAddress($this->request->post['payment_country_id'], $this->request->post['payment_zone_id']);
            $this->tax->setStoreAddress(Config::get('config_country_id'), Config::get('config_zone_id'));
            ...
            if (isset($this->request->post['from_name']) && isset($this->request->post['from_email']) && isset($this->request->post['to_name']) && isset($this->request->post['to_email']) && isset($this->request->post['amount'])) {
                ...
                if (($this->request->post['amount'] < 1) || ($this->request->post['amount'] > 1000)) {
                    $json['error']['gift_cards']['amount'] = sprintf(Lang::get('lang_error_amount'), $this->currency->format(1, false, 1), $this->currency->format(1000, false, 1) . ' ' . Config::get('config_currency'));
                }
                ...
                if (!isset($json['error']['gift_cards'])) {
                    ...
                    $this->session->data['gift_cards'][] = array(
                        ...
                        'description'       => sprintf(Lang::get('lang_text_for'), $this->currency->format($this->request->post['amount'], Config::get('config_currency')), $this->request->post['to_name']), 
                        'code'              => substr(md5(mt_rand()), 0, 10), 
                        'from_name'         => $this->request->post['from_name'], 
                        'from_email'        => $this->request->post['from_email'], 
                        'to_name'           => $this->request->post['to_name'], 
