...
class Cart extends Controller {
    ...
    public function index() {
        ...
        if ($this->cart->hasProducts() || !empty($this->session->data['gift_cards'])) {
            ...
            } elseif (!$this->cart->hasStock() && (!Config::get('config_stock_checkout') || Config::get('config_stock_warning'))) {
                $data['error_warning'] = Lang::get('lang_error_stock');
            } else {
                $data['error_warning'] = '';
            }
            
            if (Config::get('config_customer_price') && !$this->customer->isLogged()) {
            ...
            if (Config::get('config_cart_weight')) {
                $data['weight'] = $this->weight->format($this->cart->getWeight(), Config::get('config_weight_class_id'), Lang::get('lang_decimal_point'), Lang::get('lang_thousand_point'));
            } else {
                $data['weight'] = '';
            }
            ...
            $data['shipping_status'] = Config::get('shipping_status') && Config::get('shipping_estimator') && $this->cart->hasShipping();
            ...
            $this->response->setOutput(Theme::view('checkout/cart', $data));
        } else {
            Theme::setTitle(Lang::get('lang_heading_title'));
    ...
    public function add() {
        ...
        if ($product_info) {
            ...
            if (!$json) {
                ...
                $json['total'] = sprintf(Lang::get('lang_text_items'), $this->cart->countProducts() + (isset($this->session->data['gift_cards']) ? count($this->session->data['gift_cards']) : 0), $this->currency->format($total));
            } else {
