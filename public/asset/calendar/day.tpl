<div id="cal-day-box">
	<div class="row-fluid clearfix cal-row-head">
		<div class="col-xs-1 cal-cell"><%= cal.locale.time %></div>
		<div class="col-xs-11 cal-cell"><%= cal.locale.events %></div>
	</div>
	<% if(all_day.length) {%>
		<div class="row-fluid clearfix cal-day-hour">
			<div class="col-xs-1"><b><%= cal.locale.all_day %></b></div>
			<div class="col-xs-11">
	...
	<% if(before_time.length) {%>
		<div class="row-fluid clearfix cal-day-hour">
			<div class="col-xs-3"><b><%= cal.locale.before_time %></b></div>
			<div class="col-xs-5">
	...
	<div id="cal-day-panel" class="clearfix">
		...
		<% _.each(by_hour, function(event){ %>
			...
			<div class="pull-left day-event day-highlight dh-<%= event['class'] %>" style="margin-top: <%= (event.top * 30) %>px; height: <%= (event.lines * 30) %>px">
				<span class="cal-hours"><%= event.start_hour %> - <%= event.end_hour %></span><br>
				<a href="<%= event.url ? event.url : 'javascript:void(0)' %>" data-event-id="<%= event.id %>"
				   data-event-class="<%= event['class'] %>" class="event-item">
					<%= event.title %></a>
			</div>
	...
	<div class="row-fluid clearfix cal-day-hour">
		<div class="col-xs-3"><b><%= cal.locale.after_time %></b></div>
		<div class="col-xs-9">
